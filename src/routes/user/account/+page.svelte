<script lang="ts">
	...
	onMount(() => {
		try {
			...
			if (Array.isArray(data?.publicInfo) && data.publicInfo.length > 0) {
				publicInfoItems = data.publicInfo.map((row, idx) => ({
					id: Date.now() + idx,
	...
	$: availablePlatforms = platformOptions.filter(
		(platform) => !publicInfoItems.some((item) => item.platform === platform.value)
	);
	...
	async function updatePublicInfo() {
		...
		try {
			...
			fd.set(
				'items',
				JSON.stringify(validItems.map((i) => ({ platform: i.platform, link: i.link })))
			);
					...
					>
						...
						<nav class="space-y-2">
							{#each pageOptions as option}
								<button
									class="w-full flex items-center px-3 py-2 text-sm font-medium rounded-md transition-colors duration-200 {currentPage ===
									option.id
									...
									>
										...
										<span
											>{availablePlatforms.length === 0
												? m['user.account.allPlatformsAdded']()
												: m['user.account.add']()}</span
										>
									</button>
								</div>
								...
								<div class="flex justify-end items-center gap-4">
									...
									{:else if updateCount !== null}
										<p class="text-sm text-emerald-600 dark:text-emerald-400">
											{m['user.account.updatedCount']({ count: String(updateCount) })}
										</p>
									{/if}
									<button
