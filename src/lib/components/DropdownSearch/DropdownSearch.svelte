...
<script lang="ts">
	...
	export let validateFn: (query: string) => Promise<{
		valid: boolean;
		id?: string | number;
		title?: string;
		sub1?: string;
	...
	let inputClass = 'input input-bordered w-full bg-white dark:bg-gray-700';
	let showAddModal = false;
	let inputEl: HTMLInputElement;

	async function doSearch(openDropdown = true) {
		if (!inputValue.trim()) {
			results = [];
			showDropdown = false;
			return;
	...
	async function doValidate(val: string) {
		if (!val.trim()) {
			...
			valid = false;
			selectedId = null;
			inputClass = 'input input-bordered w-full bg-white dark:bg-gray-700';
		...
		if (valid && res.id !== undefined) {
			...
			dispatch('select', { id: res.id, title: res.title || inputValue });
		} else {
			selectedId = null;
			inputClass =
				'input input-bordered w-full border-red-600 focus:border-red-600 bg-white dark:bg-gray-700';
		}
	}
	...
	function handleSelect(item: DropdownItem) {
		...
		dispatch('validation', {
			valid: true,
			result: { valid: true, id: item.id, title: item.title }
		});
	...
	export function refreshSearch(openDropdown = true) {
		if (inputValue.trim()) {
			doSearch(openDropdown);
		}
	}
...
<div class="relative w-full">
	<input
		...
		oninput={handleInput}
		onfocus={() => {
			if (!disabled && (results.length || showAddButton)) showDropdown = true;
		}}
		onblur={() => setTimeout(() => (showDropdown = false), 200)}
	/>
	{#if showDropdown && (results.length > 0 || showAddButton || !loading)}
		<ul
			class="absolute left-0 right-0 bg-base-100 shadow-lg rounded-b z-50 max-h-80 overflow-auto border border-base-200"
		>
